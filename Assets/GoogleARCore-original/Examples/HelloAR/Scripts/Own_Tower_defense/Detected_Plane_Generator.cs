...
namespace GoogleARCore.Examples.Common
{
    ...
    /// </summary>
    public class Detected_Plane_Generator : MonoBehaviour
    {
        /// <summary>
        ...
        // check bool
        bool Is_buffer_clear = false;
        ArrayList DP_Prefab_list = new ArrayList();
        ...
        public void Update()
        {
            ...
            if (GameManager.Instance.Is_map_placed)
            {
                if (!Is_buffer_clear)
                {
                    // DetectedPlanePrefab gameobject을 모두 destroy
                    for(int i = 0; i < DP_Prefab_list.Count; i++)
                    {
                        Destroy((GameObject)DP_Prefab_list[i]);
                    }
                }
                return;
            }
            ...
            for (int i = 0; i < _newPlanes.Count; i++)
            {
                ...
                DP_Prefab_list.Add(planeObject);
            }
        }
    }
}
