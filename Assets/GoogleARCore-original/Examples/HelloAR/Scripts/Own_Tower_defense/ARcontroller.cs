...
namespace GoogleARCore.Examples.HelloAR
{
    ...
    public class ARcontroller : MonoBehaviour
    {
        ...
        private Anchor anchor; // 앵커 cache
        ...
        public void Update()
        {
            ...
            if (foundHit && !GameManager.Instance.Is_map_placed)
            {
                ...
                else
                {
                    ...
                    // set flag false
                    GameManager.Instance.Is_map_placed = true;
            ...
            // 3d 가상 오브젝트 선택
            // 터치를 안한다면 어차피 맨 앞에서 return;
            if (GameManager.Instance.Is_map_placed)
            {
                RaycastHit hitobject;
                Ray ray = FPS.ScreenPointToRay(Input.GetTouch(0).position);
                if(Physics.Raycast(ray, out hitobject))
                {
                    if(hitobject.transform.tag == "Node")
                    {
                        // 해당 node 색상 변경(점유되었다는 의미)
                        Renderer rend = hitobject.collider.gameObject.GetComponent<Renderer>();

                        // 해당 node에 터렛이 없고, 빌드터렛 버튼이 눌려졌고, 돈이 충분히 있다면 해당 자리에 터렛 배치
                        if (hitobject.collider.gameObject.GetComponent<Node>().turret == null && GameManager.Instance.turretToBuildidx != -1 && PlayerStats.Money >= GameManager.Instance.TurretCosts[GameManager.Instance.turretToBuildidx])
                        {
                            // 돈 차감
                            PlayerStats.Money -= GameManager.Instance.TurretCosts[GameManager.Instance.turretToBuildidx];
                            int turretidx = GameManager.Instance.turretToBuildidx;

                            var gameObject = Instantiate(GameManager.Instance.TurretPrefabs[turretidx], hitobject.collider.gameObject.transform.position, hitobject.collider.gameObject.transform.rotation);
                            hitobject.collider.gameObject.GetComponent<Node>().turret = gameObject;
                            rend.material.color = hovercolor;

                            // 다시 원상복귀.
                            GameManager.Instance.turretToBuildidx = -1;

                            // map의 anchor를 동일하게 turret의 부모로 만든다.
                            if (anchor != null) {
                                gameObject.transform.parent = anchor.transform;
                            }
